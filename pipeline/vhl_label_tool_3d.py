...
<main>
  <div class="stage">
    ...
    <p class="muted" id="hint" style="display:none;margin:8px 0 0">
      ...
      <br>Barrier marks are accepted anywhere near the heart, including on the wall &mdash;
      click the outside freely and as often as you like. They help most in the wide gaps:
      the groove between atria and ventricles, and the space between the great vessels.
 ...
 *
 * `reach` is how far to search outward for usable space, in grid steps. For a
 * chamber seed it is small: the reader is pointing at a specific lumen and a
 * long search would silently move the seed into a neighbouring one. For a
 * "not lumen" mark it is large, because the whole point of that mark is to
 * stand OUTSIDE the heart, where the nearest cavity voxel may be several
 * millimetres away — refusing those clicks made the one job the barrier exists
 * for impossible to do.
 */
function classify(point, reach) {
...
canvas.addEventListener('pointerup', e => {
  ...
  const barrier = state.tag === CFG.excludeTag;
  const hit = classify(point, barrier ? 10 : 3);
  if (!hit) {
    note(barrier ? 'Too far from the heart — click nearer its surface.'
                 : 'That is outside the heart.');
    return;
  }
  // A barrier mark is allowed to land on muscle. It is saying "nothing lives
  // here", and the reader is pointing at the outside of the heart, where the
  // nearest usable voxel may well be wall. Only chamber seeds are held to
  // landing in lumen.
  if (hit.kind === 'muscle' && !barrier) {
    note('That looks like muscle. Click the cream area.'); return;
  }
