...
- **(1c) viewer-core — done for this slice.** Orbit with no polar clamp, camera framing, the radial free cutter with solid stencil caps and an optional ghost of the removed half, direct-manipulation cut handles, two named cutter modes, the probe indicator and its control pad, the beam-dim highlight, the animated match-echo camera, and Explore mode. Outstanding: pinch-zoom and two-finger pan, per-structure show/hide, labels, measurement.
